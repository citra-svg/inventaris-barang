...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($barang_id) || $jumlah <= 0 || $harga_beli <= 0) {
        ...
        $sql = "INSERT INTO barang_masuk (barang_id, jumlah, harga_beli, total_harga, keterangan, user_id) 
                VALUES ('$barang_id', '$jumlah', '$harga_beli', '$total_harga', '$keterangan', '$user_id')";
        ...
        if (insert($sql)) {
            ...
            $error = 'Gagal mencatat barang masuk: ' . $conn->error;
        }

        // Setelah berhasil insert barang masuk
        if (insert($sql)) {
        logAktivitas($_SESSION['user_id'], 'Barang Masuk', "Barang masuk: {$barang['nama_barang']} sebanyak $jumlah");
    // ...
}
    }
}
...
if (isset($_GET['hapus'])) {
    ...
    if ($data) {
        ...
        if (delete($sql)) {
            echo "<script>
                alert('Riwayat barang masuk berhasil dihapus! Stok otomatis dikurangi.');
...
$riwayat = get("
    SELECT bm.*, b.nama_barang, b.kode_barang, u.nama_lengkap 
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4><i class="bi bi-arrow-down-circle"></i> Barang Masuk</h4>
</div>
            ...
            <div class="card-body">
                ...
                <form method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="jumlah" class="form-control" placeholder="Jumlah barang masuk" min="1" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Harga Beli per Unit <span class="text-danger">*</span></label>
                        <input type="number" name="harga_beli" class="form-control" placeholder="Harga beli per unit" min="1" required>
                    </div>
                    <div class="mb-3">
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-sm">
                        ...
                        <tbody>
                            <?php if (count($riwayat) > 0): ?>
                                <?php $no = 1; foreach ($riwayat as $item): ?>
                                    <tr>
                                        ...
                                        <td><span class="badge bg-success"><?php echo $item['jumlah']; ?></span></td>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <a href="?hapus=<?php echo $item['id']; ?>" class="btn btn-danger btn-sm" title="Hapus" onclick="return confirm('Yakin ingin menghapus riwayat ini? Stok akan otomatis berkurang.')">
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="8" class="text-center text-muted">Belum ada riwayat barang masuk</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
